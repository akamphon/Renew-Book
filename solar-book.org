...
- Passive Heating: natural means such as natural convection and radiation to collect and transfer heat

- Active Heating: use pumps or fan to enhance fluid flow and heat transfer
...
- Heat extraction rate: rate energy is removed from space (by cooling or dehumidifying)

- Heat extraction rate = Cooling load under steady state

*** วิธีการประเมินภาระความร้อน

1. Heat balance -> most detailed + provide transient bahavior
...
4. Degree-day method -> simplest method with reasonable results --> we will employ this method

*** Degree-Day Method

Assumption: people like temperature at 65 F or 18.3 C.

- Number of heating degree-days is
  \begin{align*}
    (DD)_{h} = \sum_{m}(T_{b} - T_{av})^{+}
  \end{align*}

- Number of cooling degree-days is
  \begin{align*}
    (DD)_{c} = \sum_{m}(T_{av} - T_{b})^{+}
  \end{align*}

  - $T_b$ :: base temperature = 18.3 C (65 F)
  - $T_{av}$ :: average outdoor air temperature = $(T_{\max} + T_{\min})/2$

- This info is usually available for most large cities or countries (Thailand = 5752 C-day)

\begin{align*}
  D_{h} &= (UA)(DD)_{h}
\end{align*}

- $UA$ :: heat loss characteristics of the building
\begin{align*}
  UA &= \frac{Q_{h}}{T_{i} - T_{o}}
\end{align*}

- $Q_h$ :: design peak sensible heat loss
- $T_i - T_o$ :: design indoor-outdoor temp difference
...
***** Example: Calculate required cooling load for Thailand
...
- A high heat-capacitance collector coupled to the room

- Absorbed radiation reaches the inside wall by conduction, then convected into the room

- Or convected from the outside through a designed gap
...
- Energy loss in the house is

\begin{align*}
  L_{m} = \sum \left[(UA)(T_R - T_a) - \dot{g}\right] dt = \sum \left[ (UA)(T_b - T_a) \right] dt
\end{align*}

- $UA$ :: product of overall heat transfer and surface area of building
- $\dot{g}$ :: rate of internal heat generation
- $T_R$ :: mean room temperature
- $T_b$ :: mean indoor balance temperature
- $T_a$ :: mean outdoor ambient temperature

*** Energy Balance in Thermal Storage Walled Room

- Assume steady state, energy loss = energy required
  \begin{align*}
    L_{m} &= (UA)(DD)_{b}
  \end{align*}

- \((DD)_b\) = monthly degree-days evaluated at \(T_b\)

*** Energy Loss from Building Through Thermal Storage Wall

- monthly energy loss from building through thermal storage wall is
  \begin{align*}
...
*** Overall Heat Transfer of Thermal Storage Wall
...
- $h_i$ :: internal convective heat transfer coeff \approx 8.33 W/m^{2}K
- $w$ :: wall thickness
- $k$ :: thermal conductivity of wall
- $\overline{U}_o$ :: overall heat transfer coefficient from outer wall to ambient

*** Getting Most out of Thermal Storage Wall

- Night insulation is normally installed to reduce heat loss

- This modifies $\overline{U}_o$ so that
  \begin{align*}
    \overline{U}_{o} = (1 - F)U_{o} + F\left( \frac{U_{o}}{{1 + R_{ins}U_{o}}} \right)
  \end{align*}

  - \(U_o\) :: overall coefficient with no insulation
  - \(R_{ins}\) :: thermal resistance of insulation
  - \(F\) :: fraction of time in which insulation is used

*** Monthly Energy Balance of Thermal storage Wall
\begin{align*}
...
- \(I_t\) :: monthly average radiation per unit area on the wall
- \(\overline{\tau\alpha}\) :: average transmittance of glazing and absorptance of wall
- \(T_w\) :: average outer wall temperature
- \(T_R\) :: average room temperature
- \(T_a\) :: average ambient temperature
- \(U_k\) :: overall heat transfer coefficient from outer wall to indoor space

*** Net Monthly Heat Gain through Thermal Storage Wall

\begin{align*}
  Q_{g} = U_{k}A_{w}(T_{w} - T_{R})N\Delta t
\end{align*}

***** Example: Monthly Heat Transfer Through Wall into Building
# ###### Kalogirou 6.3 ######## #

A building has a south-facing thermal storage wall with night insulation $R_{ins}$ equal to 1.52 m^2 K/W,
applied for 8 h. Estimate the monthly heat transfer through the wall into the indoor space with and
without night insulation for the month of December. The following data are given:
...
***** Solution: Monthly Heat Transfer Through Wall into Building

- Overall heat transfer from outer wall to indoor space is
  \begin{align*}
...
*** Example: Space Heating Load, Water Heating Load, and Auxiliary Energy
...
*** Solution: Space Heating Load, Water Heating Load, and Auxiliary Energy
